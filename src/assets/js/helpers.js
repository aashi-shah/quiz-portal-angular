...
// ---
// Add classes
function _addClass(cls, el = this.ROOT_EL) {
...
// ---
// Remove classes
function _removeClass(cls, el = this.ROOT_EL) {
  if (el.length !== undefined) {
...
// Toggle classes
function _toggleClass(el = this.ROOT_EL, cls1, cls2) {
...
// ---
// Has class
function _hasClass(cls, el = this.ROOT_EL) {
  let result = false
...
function _triggerWindowEvent(name) {
...
// ---
// Update style
function _updateInlineStyle(navbarHeight = 0, footerHeight = 0) {
  if (!this._styleEl) {
...
function _supportsTransitionEnd() {
...
function _getAnimationDuration(el) {
...
function _setMenuHoverState(hovered) {
...
function _bindLayoutAnimationEndEvent(modifier, cb) {
...
function _unbindLayoutAnimationEndEvent() {
  const menu = this.getMenu()
...
function _bindWindowResizeEvent() {
...
function _unbindWindowResizeEvent() {
  if (this._resizeTimeout) {
    clearTimeout(this._resizeTimeout)
    this._resizeTimeout = null
  }
...
function _unbindMenuMouseEvents() {
  if (!this._menuMouseEnter && !this._menuMouseLeave && !this._windowTouchStart) return
...
function scrollToActive(animate = false) {
  this._scrollToActive(animate)
}

// ---
// Collapse / expand layout
function  setCollapsed(collapsed = requiredParam('collapsed'), animate = true) {
  const layoutMenu = this.getLayoutMenu()
...
// ---
// Toggle layout
function toggleCollapsed(animate = true) {
  this.setCollapsed(!this.isCollapsed(), animate)
}

// ---
// Set layout positioning
function setPosition(fixed = requiredParam('fixed'), offcanvas = requiredParam('offcanvas')) {
...
function setAutoUpdate(enable = requiredParam('enable')) {
...
function on(event = requiredParam('event'), callback = requiredParam('callback')) {
...
function off(event = requiredParam('event')) {
  const [_event] = event.split('.')
  let [, ...namespace] = event.split('.')
  namespace = namespace.join('.') || null
